...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		case FunctionType::Kind::ABIEncodeWithSignature:
		{
			...
			utils().fetchFreeMemoryPointer();
			// stack now: [<selector>] <arg1> .. <argN> <memory pointer>
			...
			utils().fetchFreeMemoryPointer();
			// stack: [<selector>] <memory end> <memory ptr>
			...
			if (hasSelectorOrSignature)
			{
				...
				{
					// hash the signature
					if (auto const* selector = dynamic_cast<StringLiteralType const*>(selectorType))
					{

					}
					// Optimize in case the string is a literal of length less at most 64 bytes
					// - use the scratch space for that.
					solUnimplemented("");
				}
				else
					utils().convertType(*selectorType, FixedBytesType(4));

				// stack: <memory pointer> <selector>

				// load current memory, mask and combine the selector
				m_context.appendInlineAssembly(R"(
					let data := mload(mem_ptr)
					data := and(data, 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffff)
					mstore(mem_ptr, or(data, selector))
				)", {"mem_ptr", "selector"});
				m_context << Instruction::POP;
			}
